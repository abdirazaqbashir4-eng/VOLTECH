...
import { getOrCreateFirebaseUser, setUserRoleClaims } from "@voltech/core/firebase-admin";

// TEMPORARY, ONE-TIME-USE ENDPOINT — not linked from any UI, gated by a
// hardcoded token, and meant to be deleted immediately after a single
// successful run. It exists only because the production Render instance
// is on the free plan, which has no Shell/One-Off Jobs access to run
// `npm run db:seed` directly, and mirrors that script's logic exactly
// (same upserts, same idempotency) so it's safe to hit more than once.
const SEED_TOKEN = "voltech-seed-2f8a91c6d4b0773e5a1f9c2e6d8b4a01";
...
export async function GET(request: Request) {
  ...
  await db.shippingZone.upsert({
    ...
    create: {
      ...
      methods: { create: [{ name: "Standard Delivery", carrier: "PLATFORM", fee: 450, estimatedDaysMin: 2, estimatedDaysMax: 6 }] },
    },
  });

  const categoryNames = ["Phones & Tablets", "Electronics", "Fashion", "Home & Living", "Beauty & Personal Care", "Computing"];
  const categories: Record<string, string> = {};
  for (const name of categoryNames) {
    const slug = slugify(name);
    const cat = await db.category.upsert({ where: { slug }, update: {}, create: { name, slug, status: "ACTIVE" } });
    categories[name] = cat.id;
  }

  const adminEmail = "admin@voltech.africa";
  const adminFirebaseUser = await getOrCreateFirebaseUser({ email: adminEmail, password: "Admin123!", displayName: "VOLTECH Admin" });
  const adminUser = await db.user.upsert({
    where: { email: adminEmail },
    update: {},
    create: { email: adminEmail, firebaseUid: adminFirebaseUser.uid, fullName: "VOLTECH Admin", role: "SUPER_ADMIN", status: "ACTIVE", emailVerifiedAt: new Date() },
  });
  await setUserRoleClaims(adminFirebaseUser.uid, { role: "SUPER_ADMIN", appUserId: adminUser.id });
  log.push(`Admin: ${adminEmail} / Admin123!`);

  const sellerEmail = "seller@voltech.africa";
  const sellerFirebaseUser = await getOrCreateFirebaseUser({ email: sellerEmail, password: "Seller123!", displayName: "Jane Mwangi" });
  const sellerUser = await db.user.upsert({
    where: { email: sellerEmail },
    update: {},
    create: { email: sellerEmail, firebaseUid: sellerFirebaseUser.uid, fullName: "Jane Mwangi", role: "SELLER", status: "ACTIVE", emailVerifiedAt: new Date() },
  });
  await setUserRoleClaims(sellerFirebaseUser.uid, { role: "SELLER", appUserId: sellerUser.id });
  ...
  log.push(`Seller: ${sellerEmail} / Seller123!`);

  const customerEmail = "customer@voltech.africa";
  const customerFirebaseUser = await getOrCreateFirebaseUser({ email: customerEmail, password: "Customer123!", displayName: "Amina Yusuf" });
  const customerUser = await db.user.upsert({
    ...
    create: {
      ...
      addresses: {
        create: { label: "Home", recipientName: "Amina Yusuf", phone: "254798765432", county: "Nairobi", city: "Nairobi", street: "Moi Avenue", isDefault: true },
      },
    },
  });
  await setUserRoleClaims(customerFirebaseUser.uid, { role: "CUSTOMER", appUserId: customerUser.id });
  log.push(`Customer: ${customerEmail} / Customer123!`);
  ...
  if (!existingProduct) {
    const product = await db.product.create({
      data: {
        ...
        status: "APPROVED",
        images: { create: [{ url: "https://picsum.photos/seed/earbuds1/800/800", sortOrder: 0 }, { url: "https://picsum.photos/seed/earbuds2/800/800", sortOrder: 1 }] },
        variants: {
          create: [
    ...
    for (const v of product.variants) await db.inventory.create({ data: { productId: product.id, variantId: v.id, onHand: 50 } });
    ...
    const phone = await db.product.create({
      data: {
        ...
        status: "APPROVED",
        images: { create: [{ url: "https://picsum.photos/seed/phone1/800/800", sortOrder: 0 }, { url: "https://picsum.photos/seed/phone2/800/800", sortOrder: 1 }] },
        variants: {
          create: [
    ...
    for (const v of phone.variants) await db.inventory.create({ data: { productId: phone.id, variantId: v.id, onHand: 30 } });
    ...
    for (const v of shoes.variants) await db.inventory.create({ data: { productId: shoes.id, variantId: v.id, onHand: 20 } });

    log.push("Seeded 3 demo products for Nairobi Gadgets");
  } else {
    log.push("Demo products already existed, skipped.");
  }

  log.push(`Zones ready: ${nairobiZone.name} + Nationwide`);
  log.push(`Done. Admin id: ${adminUser.id}`);
