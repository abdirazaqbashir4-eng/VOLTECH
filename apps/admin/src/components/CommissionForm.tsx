...
export default function CommissionForm({ categories, sellers }: { categories: { id: string; name: string }[]; sellers: { id: string; storeName: string }[] }) {
  ...
  return (
    <form action={formAction} className="max-w-md space-y-3 rounded-lg border border-[var(--border)] bg-white p-4">
      ...
      {scope === "SELLER" && (
        <select name="sellerId" required className="w-full rounded-md border border-[var(--border)] px-3 py-2 text-sm">
      ...
      <input name="percentage" type="number" step="0.1" min={0} max={100} placeholder="Commission %" required className="w-full rounded-md border border-[var(--border)] px-3 py-2 text-sm" />
